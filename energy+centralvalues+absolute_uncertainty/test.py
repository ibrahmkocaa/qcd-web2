...
sadece 'energy', 'Central_Value', 'Absolute_PDF_Uncertainty' girdileri ile 'cross_section' tahmini yapan modelleri eğitir ve kaydeder.
...
Özellikler (X): energy + Central_Value + Absolute_PDF_Uncertainty
...
def train_all(file_path: str = "data_all.xlsx", models_dir: str = "models", save_individual: bool = True) -> None:
    ...
    needed = {"process", "channel", "energy", "Central_Value", "Absolute_PDF_Uncertainty", "cross_section"}
    ...
    for (process, channel), plan in MODEL_PLAN.items():
        ...
        if grp.empty:
            ...
            continue
        X = grp[["energy", "Central_Value", "Absolute_PDF_Uncertainty"]].to_numpy(dtype=float)
...
def predict(process: str, channel: int, energy: float, central_value: float, abs_pdf_unc: float, models_dir: str = "models") -> float:
    model = load_model(process, channel, models_dir=models_dir)
    X = np.array([[float(energy), float(central_value), float(abs_pdf_unc)]], dtype=float)
...
if __name__ == "__main__":
    ...
    p_pred.add_argument("abs_pdf_unc", type=float, help="Absolute_PDF_Uncertainty")
    ...
    if args.cmd == "train":
        ...
        y = predict(args.process, args.channel, args.energy, args.central_value, args.abs_pdf_unc, models_dir=args.models_dir)
        print(y)
