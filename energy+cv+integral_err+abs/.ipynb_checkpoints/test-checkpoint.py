...
[X] = ['energy', 'integral_error', 'Central_Value', 'Absolute_PDF_Uncertainty'] girdileri ile
[Y] = 'cross_section' tahmini yapan modelleri eğitir ve kaydeder.
...
- Ridge         -> Ridge
- Poly2/3/4     -> PolynomialFeatures(degree=2/3/4) + LinearRegression  (gerekirse kolayca eklenebilir)

Kaydetme yapısı (varsayılan klasör: models/):
- models/LO_91.pkl                 (tekli model)
- models/NLO_93_Ensemble.pkl       (ensemble model: Linear + Ridge)
- (İsteğe bağlı) models/NLO_93_Linear.pkl gibi tek tek modeller de kaydedilebilir.

Tahmin:
- load_model(process, channel) ile uygun modeli yükleyin
- predict(process, channel, energy, integral_error, central_value, absolute_pdf_uncertainty)
...
def build_model(name: str) -> Pipeline:
    name = name.strip()
    # Türkçe yazımlar için basit eşleme
    if name.lower() in {"lineer", "linear"}:
        return Pipeline([
            ("reg", LinearRegression())
        ])
    if name.lower() in {"ridge"}:
        return Pipeline([
            ("reg", Ridge(random_state=42))
        ])
    if name.lower().startswith("poly"):
        deg = int(name.lower().replace("poly", ""))
        return Pipeline([
...
def train_all(file_path: str = "data_all.xlsx", models_dir: str = "models", save_individual: bool = True) -> None:
    ...
    needed = {"process", "channel", "energy", "integral_error", "Central_Value", "Absolute_PDF_Uncertainty", "cross_section"}
    missing = needed - set(df.columns)
    if missing:
        raise ValueError(f"Eksik sütun(lar): {missing}")

    # Model envanteri: hangi dosya kaydedildi
    registry: Dict[str, Dict[str, Union[bool, List[str]]]] = {}
    ...
    for (process, channel), plan in MODEL_PLAN.items():
        ...
        X = grp[["energy", "integral_error", "Central_Value", "Absolute_PDF_Uncertainty"]].to_numpy(dtype=float)
...
def predict(process: str, channel: int, energy: float, integral_error: float, central_value: float, absolute_pdf_uncertainty: float, models_dir: str = "models") -> float:
    model = load_model(process, channel, models_dir=models_dir)
    X = np.array([[float(energy), float(integral_error), float(central_value), float(absolute_pdf_uncertainty)]], dtype=float)
...
if __name__ == "__main__":
    ...
    p_pred.add_argument("integral_error", type=float, help="integral_error")
    p_pred.add_argument("central_value", type=float, help="Central_Value")
    p_pred.add_argument("absolute_pdf_uncertainty", type=float, help="Absolute_PDF_Uncertainty")
