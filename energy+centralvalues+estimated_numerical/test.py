...
sadece 'energy', 'Central_Value', 'Estimated_Numerical_Uncertainty_Percent' girdileri ile 'cross_section' tahmini yapan modelleri eğitir ve kaydeder.
...
- SafeEnsemble  -> Linear + Poly2 (ensemble olarak)

Özellikler (X): energy + Central_Value + Estimated_Numerical_Uncertainty_Percent
Hedef (y): cross_section
"""
...
MODEL_PLAN: Dict[Tuple[str, int], List[str]] = {
    ...
    ("LO", 93): ["Poly2"],
    ("LO", 96): ["Linear"],
    ("LO", 97): ["Linear", "Poly2"],  # SafeEnsemble olarak tanımlandı
    ("LO", 98): ["Linear"],
...
def build_model(name: str) -> Pipeline:
    ...
    if name == "Ridge":
        return Pipeline([("reg", Ridge(random_state=42))])
...
def train_all(file_path: str = "data_all.xlsx", models_dir: str = "models", save_individual: bool = True) -> None:
    ...
    needed = {"process", "channel", "energy", "Central_Value", "Estimated_Numerical_Uncertainty_Percent", "cross_section"}
    ...
    for (process, channel), plan in MODEL_PLAN.items():
        ...
        if grp.empty:
            ...
            continue
        X = grp[["energy", "Central_Value", "Estimated_Numerical_Uncertainty_Percent"]].to_numpy(dtype=float)
        ...
        registry[f"{process}_{channel}"] = {"models": plan, "is_ensemble": len(plan) > 1}
...
def predict(process: str, channel: int, energy: float, central_value: float, est_num_unc: float, models_dir: str = "models") -> float:
    model = load_model(process, channel, models_dir=models_dir)
    X = np.array([[float(energy), float(central_value), float(est_num_unc)]], dtype=float)
...
if __name__ == "__main__":
    ...
    p_pred.add_argument("est_num_unc", type=float, help="Estimated_Numerical_Uncertainty_Percent")
    ...
    if args.cmd == "train":
        ...
        y = predict(args.process, args.channel, args.energy, args.central_value, args.est_num_unc, models_dir=args.models_dir)
        print(y)
