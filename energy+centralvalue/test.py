...
Özellikler (X): energy + Central_Value
...
def train_all(file_path: str = "data_all.xlsx", models_dir: str = "models", save_individual: bool = True) -> None:
    ...
    needed = {"process", "channel", "energy", "Central_Value", "cross_section"}
    ...
    for (process, channel), plan in MODEL_PLAN.items():
        ...
        X = grp[["energy", "Central_Value"]].to_numpy(dtype=float)
...
def predict(process: str, channel: int, energy: float, central_value: float, models_dir: str = "models") -> float:
...
if __name__ == "__main__":
    ...
    if args.cmd == "train":
        ...
        y = predict(args.process, args.channel, args.energy, args.central_value, models_dir=args.models_dir)
        print(y)
